					...
					row = `	<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuMyInvoices" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Hóa đơn cá nhân</p>
								  </div>
								</div>
							</div>
							<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuSocietyInvoices" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Hóa đơn doanh nghiệp</p>
								  </div>
								</div>
							</div>
							<div class="col col-md-4">
								<div class="card h-100">
								  <div class="card-body text-center mainmenu-subcard" id="menuCreateInvoice" style="background-color: #eeeff3; color: #2f3037;">
								    ...
								    <p class="card-text">Tạo hóa đơn</p>
								  </div>
								</div>
							</div>
						  `
			...
			if (!windowIsOpened) {
				...
				for(var i=0; i<event.data.invoices.length; i++)
				{
					...
					if (invoices.status == 'paid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-success" style="font-size: 14px;"><i class="fas fa-check-circle"></i> ĐÃ TRẢ</span></td>';
						modalstatus = '<span class="badge bg-success" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-check-circle"></i> ĐÃ TRẢ</span>';
						payment_status = `<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">TRẢ LÚC: ${invoices.paid_date}</span>`;
					}
					else if (invoices.status == 'unpaid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-danger" style="font-size: 14px;"><i class="fas fa-times-circle"></i> CHƯA TRẢ</span></td>';
						modalstatus = '<span class="badge bg-danger" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-times-circle"></i> CHƯA TRẢ</span>';
						...
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-info" style="font-size: 14px;"><i class="fas fa-clock"></i> TỰ ĐỘNG TRẢ</span></td>';
						modalstatus = '<span class="badge bg-info" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-clock"></i> TỰ ĐỘNG TRẢ</span>';
						payment_status = `<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">TỰ ĐỘNG TRẢ LÚC: ${invoices.paid_date}</span>`;
					}
					else if (invoices.status == 'cancelled') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-secondary" style="font-size: 14px;"><i class="fas fa-minus-circle"></i> ĐÃ HỦY</span></td>';
						modalstatus = '<span class="badge bg-secondary" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-minus-circle"></i> ĐÃ HỦY</span>';
						payment_status = '<span style="font-size: 20px; color: #2f3037; text-transform: uppercase; font-weight: 600;">ĐÃ HỦY</span>'
					}

					if (invoices.limit_pay_date == 'N/A') {
						limit_pay_date = ''
					} else {
						limit_pay_date = `<h6>Hạn trả: ${invoices.limit_pay_date}</h6>`
					}
					...
					row += `
						<tr>
							...
							<td class="text-center align-middle"><button type="button" class="btn btn-blue showInvoice" style="border-radius: 10px; flex-basis: 100%;" data-toggle="modal" data-target="#showInvoiceModal${invoices.id}" data-invoiceId="${invoices.id}""><i class="fas fa-eye"></i> XEM</button></td>
						</tr>
					`;
									...
									<div class="modal-dialog modal-dialog-centered">
										<div class="modal-content myinvoices_modal-content">
											<div class="modal-body p-4">
												<span class="menutitle">Hóa đơn #${invoices.id}</span>
												${modalstatus}
												<h6 class="" id="" style="">Đã gửi: ${invoices.sent_date}</h6>
												${limit_pay_date}
												<h6 class="" id="" style="margin-top: 20px;">Đến: <span style="color: #2f3037;">${invoices.receiver_name}</span></h6>
												<h6 class="" id="" style="">Từ: <span style="color: #2f3037;">${invoices.author_name}</span></h6>
												<hr>
												...
												<hr>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px;">Số tiền</span> <span class="" id="" style="font-size: 18px;">${Math.round(invoices.invoice_value - Math.round(invoices.invoice_value * (event.data.VAT/100))).toLocaleString()} &euro;</span>
												</div>
												<div class="d-flex justify-content-between">
												...
												<br>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px; font-weight: 700;">Tổng tiền</span> <span class="" id="" style="font-size: 18px;">${Math.round(invoices.invoice_value).toLocaleString()} &euro;</span>
												</div>
												<br>
			...
			if (!windowIsOpened) {
				...
				for(var i=0; i<event.data.invoices.length; i++)
				{
					...
					if (invoices.status == 'paid') {
						...
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-danger" style="font-size: 14px;"><i class="fas fa-times-circle"></i> QUÁ HẠN</span></td>';
						modalstatus = '<span class="badge bg-danger" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-times-circle"></i> QUÁ HẠN</span>';
						payment_status = `<button type="button" id="" class="btn btn-red flex-grow-1 cancelInvoice" style="border-radius: 10px; flex-basis: 100%;" data-invoiceId="${invoices.id}" data-invoiceMoney="${invoices.invoice_value}" data-bs-dismiss="modal"><i class="fas fa-times-circle"></i> HỦY HÓA ĐƠN</button>`;
					}
					else if (invoices.status == 'autopaid') {
						tablestatus = '<td class="text-center align-middle"><span class="badge bg-info" style="font-size: 14px;"><i class="fas fa-clock"></i> TỰ ĐỘNG TRẢ</span></td>';
						modalstatus = '<span class="badge bg-info" style="position: absolute; right: 5%; top: 5%; font-size: 18px;"><i class="fas fa-clock"></i> TỰ ĐỘNG TRẢ</span>';
									...
									<div class="modal-dialog modal-dialog-centered">
										<div class="modal-content myinvoices_modal-content">
											<div class="modal-body p-4">
												<span class="menutitle">Hóa đơn #${invoices.id}</span>
												${modalstatus}
												<h6 class="" id="" style="">Ngày gửi: ${invoices.sent_date}</h6>
												${limit_pay_date}
												<h6 class="" id="" style="margin-top: 20px;">Đến: <span style="color: #2f3037;">${invoices.receiver_name}</span></h6>
												<h6 class="" id="" style="">Từ: <span style="color: #2f3037;">${invoices.author_name}</span></h6>
												<hr>
												...
												<hr>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px;">Tổng tiền</span> <span class="" id="" style="font-size: 18px;">${Math.round(invoices.invoice_value - Math.round(invoices.invoice_value * (event.data.VAT/100))).toLocaleString()} &euro;</span>
												</div>
												<div class="d-flex justify-content-between">
													<span class="" id="" style="color: #2f3037; font-size: 20px;">VAT (${event.data.VAT}%)</span> <span class="" id="" style="font-size: 18px;">${Math.round(invoices.invoice_value * (event.data.VAT/100)).toLocaleString()} &euro;</span>
												</div>
...
$(document).on('click', '#sendInvoice', function() {
	...
	if (document.getElementById("invoice_notes").value === "") {
		invoice_notes = "Không có ghi chú.";
	}
	...
	if(!invoice_value || !invoice_item) {
		...
		if(invoice_value >= 0) {
			...
			$.post('http://okokBilling/action', JSON.stringify({
				...
				invoice_notes: "Ghi chú: " + invoice_notes
			}));
