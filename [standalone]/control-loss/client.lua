...
function IsWeatherAccurate(weather)
    ...
    for _, var in pairs(accurateWeather) do
        if (var == weather) then
            return true
        end
    end
...
function IsClassDisabled(vehicle)
    ...
    for _, var in pairs(disabledClasses) do
...
function IsVehicleDisabled(vehicle)
    for _, var in pairs(config.disabledVehicles) do
...
function ShakePad()
    if not (isPadShaking) then
        ...
        local shakeDuration = math.random(1, 2) * 1000
...
RegisterNetEvent('VehicleControlLoss', function(vehicle)
