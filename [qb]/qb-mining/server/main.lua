...
RegisterNetEvent('tr-mining:BuyPickaxe', function()
    ...
    if not pickaxe then
        Player.Functions.AddItem('mining_pickaxe', 1, false, info)
...
QBCore.Functions.CreateCallback('tr-mining:pickaxe', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player then
        if Player.Functions.GetItemByName("mining_pickaxe") then
            cb(true)
        else
            cb(false)
        end
...
-- RegisterNetEvent('tr-mining:decreaseQuality', function(amount)
--     local Player = QBCore.Functions.GetPlayer(source)
--     local item = Player.Functions.GetItemByName('mining_pickaxe')
--     if item then
--         local quality = item.info.quality
--         local itemSlot = item.slot
--         local newQuality = tonumber(quality - amount)
--         Player.PlayerData.items[itemSlot].info.quality = newQuality
...
--     -- --         Player.Functions.RemoveItem('mining_pickaxe', 1)
--     -- --         local info = {
--     -- --             quality = item.info.quality - amount
--     -- --         }
--     -- --         Player.Functions.AddItem('mining_pickaxe', 1, false, info)
--     -- --     end
...
--     -- --         Player.Functions.RemoveItem('mining_pickaxe', 1)
--     -- --         QBCore.Functions.Notiyfy(source, "Xẻng của bạn đã hư, hãy đi mua cái mới", 'error')
--     -- --     end
--     -- -- end
-- end)
...
RegisterServerEvent('tr-mining:receivedStone', function()
    ...
    if exports['ps-buffs']:HasBuff("luck") then
        mineStone = mineStone + 1
    end
    local lucky = math.random(1, 100)
    local info = {
        url = "https://cdn.discordapp.com/attachments/1002855360572436550/1024008847913922671/dieman5.png"
    }
...
RegisterNetEvent('tr-mining:setMiningTimer', function()
    if not mining then
        ...
        CreateThread(function()
            Wait(Config.Timeout)
            for k, _ in pairs(Config.MiningLocation) do
