...
local function StartMining(mining)
  ...
  TriggerServerEvent('weapons:server:decreaseQuality', 'mining_pickaxe', 2)
  ...
  CreateThread(function()
      while isMining do
          ...
          TaskPlayAnim(trClassic, animDict, animName, 8.0, -8.0, -1, 2, 0, 0, 0, 0)
...
RegisterNetEvent('tr-mining:miningwithaxe', function()
  ...
  local trpickaxeprop = CreateObject(GetHashKey("prop_tool_pickaxe"), 0, 0, 0, true, true, true)        
  ...
  AttachEntityToEntity(trpickaxeprop, ped, GetPedBoneIndex(ped, 57005), 0.15, 0.05, 0.0, -65.0, 0.0, 0.0, 1, 1, 0, 0, 2, 1)
...
CreateThread(function() 
  while true do
    ...
    for _, v in pairs(Config.MiningLocation) do
      if #(pos - vector3(v.coords.x, v.coords.y, v.coords.z)) <= 5 then
        sleep = 5
