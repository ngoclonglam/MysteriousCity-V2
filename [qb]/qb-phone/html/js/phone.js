...
QB.Phone.Functions.SetupRecentCalls = function(recentcalls) {
    ...
    $.each(recentcalls, function(i, recentCall){
        // Fixed issue that recent call from unknown number that not have name
        const FirstLetter = typeof recentCall.name === 'string' ? (recentCall.name).charAt(0) : '?';
...
QB.Phone.Functions.SetupCurrentCall = function(cData) {
    if (cData.InCall) {
        ...
        $(".phone-currentcall-contact").html("From "+cData.TargetData.name);
    } else {
