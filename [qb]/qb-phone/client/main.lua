...
local function IsNumberInContacts(num)
    ...
    for _, v in pairs(PhoneData.Contacts) do
        -- Fix issue that cannot show contact name of recent call
...
local function GetKeyByNumber(Number)
    ...
    if PhoneData.Chats then
        for k, v in pairs(PhoneData.Chats) do
            -- Fixed issue that cannot reply message to sender
...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify(Lang:t('error.not_have_phone'), "error")
        end
    end)
end
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify(Lang:t('error.action_not_available'), "error")
        end
    end
end)
...
if findVehFromPlateAndLocate(veh.plate) then
    QBCore.Functions.Notify(Lang:t('gps.vehicle_marked'), "success")
else
    QBCore.Functions.Notify(Lang:t('gps.vehicle_cannot_located'), "error")
end
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify(Lang:t('gps.gps_set_with_title', {value = data.alert.title,}))
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify(Lang:t('gps.gps_set_to', { value = data.HouseData.HouseData.adress}), "success")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify(Lang:t('gps.gps_set'), 'success')
end)
...
--[[ exports['ch-stuck']:SendDiscordNotify(src)
        QBCore.Functions.Progressbar("PlayerTeleport", "Teleporting to Ground...", Config.Time, false, true, {
            ...
            SetEntityCoords(PlayerPedId(), Config.TeleportLocation.x, Config.TeleportLocation.y, Config.TeleportLocation.z)
            QBCore.Functions.Notify("You were teleported to ground!")
            DoScreenFadeIn(650)
            if Config.SendMessages then
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Phone",
            text = Lang:t('contact.new_suggested'),
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
...
RegisterNetEvent('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        QBCore.Functions.Notify(Lang:t('error.no_one_nearby'), "error")
    end
end)
...
RegisterNetEvent('qb-pings:client:SendLocation', function(PingData, SenderData)
    QBCore.Functions.Notify(Lang:t('gps.location_blip_set'), 'success')
...
RegisterNUICallback('CasinoPhoneJobCenter', function(data)
    ...
    elseif data.action == 2 or data.action == 3 then
        SetNewWaypoint(data.x, data.y)
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
                title = "GPS Set",
                text = Lang:t('gps.gps_set'),
                icon = "fas fa-map-pin",
                color = "#f5a15b",
                timeout = 1500,
            },
