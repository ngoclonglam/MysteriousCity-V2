...
local WebHook = "https://discord.com/api/webhooks/990160108057165844/OIEVzw_mEaJaLSZPlao_flGtsJILjSWEXzcpisJU4BPfgZlODDiauP3kRoNn6_wznmYr"
local bannedCharacters = {'%','$',';'}
local Garages = Garages or {}
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    if SenderPly and Config.BillingCommissions[society] then
        ...
        SenderPly.Functions.AddMoney('bank', commission, "PayInvoice")
        invoiceMailData = {
    ...
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "lawyer" or Player.PlayerData.job.name == "realestate" or
                ...
                Player.PlayerData.job.name == "tunner") and
...
QBCore.Commands.Add("setmetadata", "Set Player Metadata (God)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if args[1] then
