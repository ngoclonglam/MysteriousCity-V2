    ...
    while true do
        if preferencesData then
            if tonumber(preferencesData.refreshRate) then
                Wait(tonumber(preferencesData.refreshRate))
            else
                Wait(Config.DefaultRefreshRate)
            end
        else
            Wait(Config.DefaultRefreshRate)
        end
...
CreateThread(function()
    ...
    if vehicle ~= 0 and CarHudVisible then
        ...
        if nitro[GetVehicleNumberPlateText(vehicle)] then
...
CreateThread(function()
    while true do
		...
		local var1, var2 = GetStreetNameAtCoord(position.x, position.y, position.z, ResultAsInteger(), ResultAsInteger())
